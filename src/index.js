...
function GameServer() {
    ...
    this.verifyGameClientRequest = function (verifier, data, onVerificationDone) {
        ...
        try {
            ...
            let isVerificationPerformed = false;
            ...
            ll.debug(`socket: auth-res verification error: ${e.message}`, e);
            onVerificationDone({
                verifies: false,
                network,
                message
...
// the user want to go to this land
            ...
            socket.on("api", (data, callback) => {
                if (data.requestType === "getstate") {
                    // return the whole madax game state
                    callback(server.gameState)
                }
                if (data.requestType === "verify") {
                    // verify an arbitrary game message
                    ll.debug("socket: api:verify", JSON.stringify(data, null, 2));
                    server.verifyGameClientRequest(
                        verifier,
                        data.payload.data,
                        ({
                             verifies,
                            ...
                            ll.debug("api:verify", {
                                data, verifies, originatorAddress, network, message, signingResponse, authSocketId
                            });
                            return callback({
                              data, verifies, originatorAddress, network, message, signingResponse, authSocketId
                            });
